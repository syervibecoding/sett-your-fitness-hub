...
- 📨 claude → engine_chat — SPEC "FALLBACK-FIRST" (2026-06-17): `docs/prescription/fallback-first-prescriptions-spec.md`. Tornar TODAS as prescrições completas em fallback determinístico (IA só refina texto, Haiku, gateável) p/ baratear. Ordem: A) CARDIO (criar `_shared/prescription/cardio/cardioEngine.ts` + `assertPlanComplete` no insert do `ai-running-plan` — para o sangramento da casca vazia), B) FORÇA (cutover do `engine.ts` modo 'on' da flag PRESCRIPTION_ENGINE_V1), C) NUTRIÇÃO (`_shared/nutrition/engine.ts` + remover 503 + bug schema meals), D) IA-enriquecimento+gating (migration aditiva `company_ai_config`). Decisões pendentes do Matheus no doc (refino default, fórmula TMB, limpeza das 11 cascas, síncrono/assíncrono, unidades natação/ciclismo). **Claude não edita edges/migrations** — só render do aluno.
